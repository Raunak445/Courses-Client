...
const EditFaq = (props: Props) => {
  ...
  const [editLayout, {error, isLoading:isLayoutSuccess }] = useEditLayoutMutation(
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      setOriginalQuestions(data?.layout?.faq); // Store the original questions for comparison
    }
  ...
  const toggleQuestion = (id: any) => {
    setQuestions((prevQ) =>
      prevQ.map((q) => (q._id === id ? { ...q, active: !q.active } : q))
    );
  };

  const handleQuestionChange = (id: any, value: string) => {
    setQuestions((prevQ) =>
      prevQ.map((q) => (q._id === id ? { ...q, question: value } : q))
    );
  };

  const handleAnswerChange = (id: any, value: string) => {
    setQuestions((prevA) =>
      prevA.map((q) => (q._id === id ? { ...q, answer: value } : q))
    );
  };
  ...
  return (
   <>
   {
    isLoading?(<Loader/>):(
          ...
          {questions?.map((q: any, index: number) => (
            ...
            >
              <dt className="text-lg">
                <button
                  className="flex items-start dark:text-white text-black justify-between w-full text-left
                  focus:outline-none"
      ...
      <div
        className={`${styles.button} !w-[100px] !min-h-[40px] !h-[40px] dark:text-white text-black bg-green-400 
          ${
            areQuestionsUnchanged(originalQuestions, questions) ||
            isAnyQuestionEmpty(questions)
              ? "!cursor-not-allowed"
              : "cursor-pointer !bg-[#42d383]"
          }!rounded fixed bottom-1 right-1 `}
        onClick={
          areQuestionsUnchanged(originalQuestions, questions) ||
          isAnyQuestionEmpty(questions)
            ? () => null
